...
import {
  Plus, Filter, Eye, FileText, Zap, Download, CheckCircle,
  XCircle, Archive, AlertCircle, Clock, TrendingUp, DollarSign, Send,
} from 'lucide-react';
import { bordereauxApi } from '../../api/bordereaux.api';
import type { Bordereau, BordereauFilters, BordereauStatus, BordereauType } from '../../types/bordereau.types';
...
const TYPE_LABELS: Record<BordereauType, string> = {
  CESSION_CEDANTE: 'Cession Cédante',
  CESSION_REASSUREUR: 'Cession Réassureur',
  SINISTRE_FACULTATIVE: 'Sinistre Facultative',
...
export default function BordereauxList() {
  ...
  const { data: stats } = useQuery({
    queryKey: ['bordereaux-stats', filters.cedanteId, filters.reassureurCode],
    queryFn: () => bordereauxApi.getStatistics({ cedanteId: filters.cedanteId, reassureurCode: filters.reassureurCode }),
  });
  ...
  const invalidate = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ['bordereaux-stats'] });
    queryClient.invalidateQueries({ queryKey: ['bordereaux-overdue'] });
    queryClient.invalidateQueries({ queryKey: ['bordereaux-due-soon'] });
    setSelectedIds([]);
  };
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">Total Bordereaux</p><p className="text-3xl font-bold mt-1">{stats?.data?.total || 0}</p></div>
            <FileText className="text-blue-500" size={32} />
          </div>
        </Card>
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">Solde Total</p><p className="text-2xl font-bold mt-1">{(stats?.data?.totalSolde || 0).toLocaleString()}</p></div>
            <TrendingUp className="text-green-500" size={32} />
          </div>
        </Card>
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">En Retard</p><p className="text-3xl font-bold mt-1 text-red-600">{overdue?.data?.length || 0}</p></div>
            <AlertCircle className="text-red-500" size={32} />
          </div>
        </Card>
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600">À Échoir (7j)</p><p className="text-3xl font-bold mt-1 text-orange-600">{dueSoon?.data?.length || 0}</p></div>
      ...
      <Card className="p-4">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <select
            value={filters.type || ''}
            onChange={(e) => setFilters({ ...filters, type: (e.target.value || undefined) as BordereauType, page: 1 })}
          ...
          <select
            ...
            onChange={(e) => setFilters({ ...filters, statut: (e.target.value || undefined) as BordereauStatus, page: 1 })}
            className="border rounded-lg px-3 py-2 text-sm"
          >
            <option value="">Tous les statuts</option>
            {Object.entries(STATUS_CONFIG).map(([value, { label }]) => <option key={value} value={value}>{label}</option>)}
          </select>

          <input
            type="text"
            placeholder="Rechercher (numéro, cédante)..."
          ...
          <select
            ...
            onChange={(e) => setFilters({ ...filters, overdue: e.target.value || undefined, page: 1 })}
            className="border rounded-lg px-3 py-2 text-sm"
          >
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="px-4 py-3 text-left">
                    <input type="checkbox" checked={selectedIds.length === rows.length && rows.length > 0} onChange={handleSelectAll} className="rounded" />
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">N° Bordereau</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Type</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cédante / Réassureur</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {rows.map((b: Bordereau) => {
                  const StatusIcon = STATUS_CONFIG[b.statut].icon;
                  return (
                    <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-4">
                        <input type="checkbox" checked={selectedIds.includes(b.id)} onChange={() => handleSelectOne(b.id)} className="rounded" />
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">{b.numero}</div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <StatusIcon size={16} />
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_CONFIG[b.statut].color}`}>{STATUS_CONFIG[b.statut].label}</span>
                          {b.isOverdue && <Badge className="bg-red-100 text-red-800 text-xs">Retard</Badge>}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-center gap-2">
                          <Button size="sm" variant="ghost" onClick={() => navigate(`/bordereaux/${b.id}`)} title="Voir détails"><Eye size={16} /></Button>
                          <Button size="sm" variant="ghost" onClick={() => downloadPdfMutation.mutate(b.id)} disabled={downloadPdfMutation.isPending} title="Télécharger PDF"><Download size={16} /></Button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                {rows.length === 0 && (
                  <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-500">Aucun bordereau trouvé</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <BordereauCreateModal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} />
      <BordereauGenerateModal isOpen={isGenerateModalOpen} onClose={() => setIsGenerateModalOpen(false)} />
    </div>
  );
}