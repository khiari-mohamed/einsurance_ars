...
export default function AffaireWorkflowActions({ affaire }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <p className="text-[11px] text-muted-foreground uppercase font-medium mb-1">Statut actuel</p>
        <p className="text-[14px] font-semibold text-foreground">{statutLabels[affaire.statut]}</p>
      </div>

      {error && (
        <div className="p-2.5 bg-destructive/10 border border-destructive/30 rounded-lg text-[12px] text-destructive flex items-start gap-2">
      ...
      {nextOptions.length === 0 ? (
        <p className="text-[12px] text-muted-foreground">Statut terminal — aucune transition possible.</p>
      ) : (
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-[13px] font-medium transition-colors disabled:opacity-50 ${
                  isForward ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'bg-card border border-border text-muted-foreground hover:bg-muted'
                }`}
              >
      ...
      {affaire.type === AffaireType.FACULTATIVE && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-card border border-border text-foreground rounded-lg hover:bg-muted transition-colors text-[13px] font-medium disabled:opacity-50"
        >
          <RotateCcw size={16} />
